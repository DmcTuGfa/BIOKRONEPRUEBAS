...
export interface StateData {
  ...
  observations: string
  agents?: ZoneAgent[]   // multiple agents per state
}
...
const coverageData: Record<string, StateData> = {
  ...
  Chihuahua:      { name: "Chihuahua",      status: "distribuidor", observations: "Distribuidor autorizado: Zona Norte.", agents: [{ zone: "ZONA NORTE", whatsapp: "4611195234" }] },
  Coahuila:       { name: "Coahuila",       status: "distribuidor", observations: "Distribuidor autorizado: Zona Norte.", agents: [{ zone: "ZONA NORTE", whatsapp: "4611195234" }] },
  "Nuevo León":   { name: "Nuevo León",     status: "distribuidor", observations: "Distribuidor autorizado: Zona Norte.", agents: [{ zone: "ZONA NORTE", whatsapp: "4611195234" }] },
  Tamaulipas:     { name: "Tamaulipas",     status: "distribuidor", observations: "Distribuidor autorizado: Zona Norte.", agents: [{ zone: "ZONA NORTE", whatsapp: "4611195234" }] },
  Durango:        { name: "Durango",        status: "distribuidor", observations: "Distribuidor autorizado: Zona Norte.", agents: [{ zone: "ZONA NORTE", whatsapp: "4611195234" }] },
  "San Luis Potosí": { name: "San Luis Potosí", status: "distribuidor", observations: "Red de distribuidores certificados.", agents: [{ zone: "ZONA NORTE", whatsapp: "4611195234" }] },
...
const getStateColor = (stateName: string, isHovered: boolean, isDark: boolean) => {
  ...
  switch (d.status) {
    case "activa":       return isHovered ? "#15803d" : "#22c55e"
    case "distribuidor": return isHovered ? "#65a30d" : "#84cc16"
    case "proximamente": return isHovered ? (isDark ? "#4b5563" : "#9ca3af") : (isDark ? "#374151" : "#d1d5db")
    default:             return isHovered ? (isDark ? "#4b5563" : "#d1d5db") : (isDark ? "#374151" : "#e5e7eb")
  }
}
...
export function MexicoMap({ onStateClick }: MexicoMapProps) {
  ...
  if (typeof window !== "undefined") {
    ...
    if (isDark !== dark) setIsDark(dark)
  }

  return (
    <div className="w-full">
      <ComposableMap projection="geoMercator" projectionConfig={{ scale: 1200, center: [-102, 23] }}
        className="w-full h-auto" style={{ maxHeight: "500px" }}>
        <ZoomableGroup>
              ...
              geographies.map((geo) => {
                ...
                return (
                  <Geography key={geo.rsmKey} geography={geo}
                    ...
                    stroke={isDark ? "#1f2937" : "#ffffff"} strokeWidth={0.5}
                    style={{ default: { outline: "none" }, hover: { outline: "none", cursor: "pointer" }, pressed: { outline: "none" } }}
                    onMouseEnter={() => setHoveredState(stateName)}
                    onMouseLeave={() => setHoveredState(null)}
                    onClick={() => {
                      const d = coverageData[stateName]
                      onStateClick(d ?? { name: stateName, status: "proximamente", observations: "Información no disponible." })
                    }}
                  />
                )
              })
