...
export async function POST(req: NextRequest) {
  try {
    ...
    const products = await prisma.product.findMany({
      where: {
        active: true,
        OR: [{ id: { in: lookups } }, { slug: { in: lookups } }],
      },
    })
    ...
    for (const item of matchedItems) {
      ...
      if (item.product.priceMxn <= 0) {
        return NextResponse.json(
          { error: `${item.product.name} no tiene precio configurado. Contacta a ventas.` },
          { status: 400 }
        )
      }
    }
    ...
    const stripeSession = await stripe.checkout.sessions.create({
      ...
      metadata: { folio, userId: session.userId },
      success_url: `${appUrl}/tienda/success?folio=${folio}`,
      cancel_url:  `${appUrl}/tienda/checkout?cancelled=1`,
    })

    // Usar transacción atómica para crear el pedido y descontar stock
    // La restricción UNIQUE en stripeSessionId previene pedidos duplicados
    ...
    // Pedido duplicado (stripeSessionId ya existe)
