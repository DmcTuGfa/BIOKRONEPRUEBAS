...
import { ShoppingCart, CheckCircle2, ChevronRight, Minus, Plus, Leaf, Bug, Shield, FlaskConical, Sprout, FileText, CreditCard, Loader2 } from "lucide-react"
...
export default function ProductoPage() {
  ...
  const handleAddToCart = () => {
    addToCart({ id: product.id, name: product.name, slug: product.slug, price: product.priceMxn, priceDisplay: `$${(product.priceMxn/100).toFixed(2)}`, presentation: product.presentation, image: product.image, category: product.category }, qty)
    setAdded(true)
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 mb-16">
            <div className="relative aspect-square rounded-2xl overflow-hidden bg-white border border-border shadow-sm">
              <Image src={product.image} alt={product.name} fill className="object-contain p-4" sizes="(max-width:1024px) 100vw, 50vw" priority />
              {product.stock === 0 && (
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                  <span className="text-white text-lg font-bold bg-black/60 px-4 py-2 rounded-xl">Agotado</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-baseline gap-2 mb-8">
                ...
                {hasPrice && <span className="text-muted-foreground">MXN / unidad</span>}
              </div>
              ...
              {hasPrice && product.stock > 0 ? (
                <>
                  <div className="flex items-center gap-4 mb-4">
                    <div className="flex items-center border border-border rounded-xl overflow-hidden bg-card">
                      <button onClick={() => setQty(q => Math.max(1, q - 1))} className="px-4 py-3 hover:bg-muted transition-colors active:scale-95">
                        <Minus className="h-4 w-4 text-foreground" />
                      </button>
                      <span className="px-5 py-3 font-bold text-foreground text-xl min-w-[4rem] text-center select-none">{qty}</span>
                      <button onClick={() => setQty(q => Math.min(product.stock, q + 1))} className="px-4 py-3 hover:bg-muted transition-colors active:scale-95">
                        <Plus className="h-4 w-4 text-foreground" />
                      </button>
                    </div>
                    <Button size="lg" onClick={handleAddToCart} className={`flex-1 h-12 text-base ${added ? "bg-green-600 hover:bg-green-700" : ""}`} disabled={added}>
                      {added ? <><CheckCircle2 className="h-5 w-5 mr-2" />¡Agregado!</>
                        : <><ShoppingCart className="h-5 w-5 mr-2" />Agregar {qty > 1 ? `${qty} unidades` : "al carrito"}</>}
                    </Button>
                  </div>
                  {inCart && <p className="text-sm text-muted-foreground mb-4">Ya tienes <strong className="text-foreground">{inCart.quantity}</strong> en el carrito.</p>}
                  <Button variant="outline" size="lg" className="h-12 text-base mb-6" onClick={() => window.location.href = "/tienda/checkout"}>
                    <CreditCard className="h-5 w-5 mr-2" />Comprar ahora
                  </Button>
                  <p className="text-xs text-muted-foreground">Stock disponible: {product.stock} unidades</p>
                </>
              ) : !hasPrice ? (
                <Button size="lg" variant="outline" className="h-12 text-base mb-6" asChild>
                ...
                <div className="p-4 rounded-xl bg-muted text-muted-foreground text-sm">Producto agotado momentáneamente.</div>
              )}

              <div className="mt-6 pt-6 border-t border-border grid grid-cols-2 gap-3 text-sm text-muted-foreground">
          ...
          {(product.fullDescription || (product.benefits?.length > 0) || product.application) && (
            <div className="border border-border rounded-2xl overflow-hidden mb-10">
              <div className="flex border-b border-border bg-muted/30 overflow-x-auto">
                {[
                  { id: "descripcion", label: "Descripción completa", icon: FileText, show: !!product.fullDescription },
                  { id: "beneficios",  label: "Beneficios",           icon: Sprout,   show: product.benefits?.length > 0 },
                  { id: "aplicacion",  label: "Modo de aplicación",   icon: FlaskConical, show: !!product.application },
